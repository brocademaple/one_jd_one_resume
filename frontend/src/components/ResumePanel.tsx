...
import { FileText, Edit3, Save, X, Download, ChevronDown, Expand } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { Resume } from '../types';
import { updateResume, getExportPdfUrl, getExportWordUrl, getPreviewPdfUrl } from '../api';
      ...
      <div className="flex items-center gap-1 ml-2">
        ...
        {editing ? <><button className="p-1.5 rounded-lg bg-primary-600 text-white" disabled={saving} onClick={async () => { setSaving(true); const updated = await updateResume(resume.id, { title: editTitle, content: editContent }); onResumeUpdated(updated); setSaving(false); setEditing(false); }}>{saving ? '保存中' : <><Save size={13} className="inline mr-1" />保存</>}</button><button className="p-1.5" onClick={() => setEditing(false)}><X size={14} /></button></> : <>
          ...
          <div className="relative">
            <button className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-gray-500 hover:bg-gray-100 text-xs" onClick={() => setShowExportMenu(v => !v)}><Download size={13} />导出<ChevronDown size={11} /></button>
            {showExportMenu && <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-10 min-w-[120px]">
              <a href={getExportWordUrl(resume.id)} target="_blank" className="block px-3 py-2 text-xs hover:bg-gray-50" onClick={() => setShowExportMenu(false)}>导出 Word</a>
              <a href={getExportPdfUrl(resume.id)} target="_blank" className="block px-3 py-2 text-xs hover:bg-gray-50" onClick={() => setShowExportMenu(false)}>导出 PDF</a>
            </div>}
          </div>
        </>}
      </div>
